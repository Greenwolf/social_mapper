...
class Linkedinfinder(object):
    ...
    def doLogin(self, username, password):
        ...
        if(self.driver.title.encode('ascii', 'replace').startswith(bytes("LinkedIn Login", 'utf-8'))):
            ...
            try:
                lnkUsername = self.driver.find_element_by_id(
                    "session_key-login")
            except:
                try:
            ...
            try:
                lnkPassword = self.driver.find_element_by_id(
    ...
    def getLinkedinProfiles(self, first_name, last_name, username, password):
        ...
        url = "https://www.linkedin.com/search/results/people/?keywords=" + \
            first_name + "%20" + last_name + "&origin=SWITCH_SEARCH_VERTICAL"
        self.driver.get(url)
        sleep(3)
                        ...
                        if "login" in self.driver.current_url:
                            ...
                            if "login" in self.driver.current_url:
                                print(
                                    "LinkedIn Timeout Error, session has expired and attempts to reestablish have failed")
                                return picturelist
                        searchresponse = self.driver.page_source.encode(
                            'utf-8')
                        soupParser = BeautifulSoup(
                            searchresponse, 'html.parser')
        ...
        for element in soupParser.find_all('div', {'class': 'search-result__image-wrapper'}):
            ...
            try:
                ...
                picturelist.append(
                    ["https://linkedin.com" + link, profilepic, 1.0])
            except:
                continue
