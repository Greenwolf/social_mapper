...
class Twitterfinder(object):
    ...
    def doLogin(self, username, password):
        ...
        if(self.driver.title.encode('ascii', 'replace').startswith(bytes("Login on", 'utf-8'))):
            ...
            try:
                twUsername = self.driver.find_element_by_class_name(
                    "js-username-field")
            except:
            ...
            try:
                twPassword = self.driver.find_element_by_class_name(
                    "js-password-field")
            except:
    ...
    def getTwitterProfiles(self, first_name, last_name):
        ...
        url = "https://twitter.com/search?q=" + first_name + \
            "%20" + last_name + "&src=typd&f=user"
        self.driver.get(url)
        sleep(3)
        ...
        for element in soupParser.find_all('div', {'class': 'css-18t94o4 css-1dbjc4n r-1j3t67a r-1w50u8q r-o7ynqc r-1j63xyz'}):
            try:
                ...
                profilepic = smallpic.replace(
                    "_reasonably_small.", "_400x400.")
                picturelist.append(
                    ["https://twitter.com" + link, profilepic, 1.0])
            except:
                print("Error")
                continue
        return picturelist
