...
class Instagramfinder(object):
    ...
    def doLogin(self, username, password):
        ...
        titleString = ''.join(
            [i if ord(i) < 128 else ' ' for i in self.driver.title])

        if(titleString.startswith("Login")):
            print("\n[+] Instagram Login Page loaded successfully [+]")
            try:
                instagramUsername = self.driver.find_element_by_xpath(
                    "//input[@name='username']")
            except:
            ...
            try:
                instagramPassword = self.driver.find_element_by_xpath(
                    "//input[@name='password']")
            except:
        ...
        try:
            ...
            for element in soupParser.find_all('a', {'class': 'yCE8d'}):
                ...
                try:
                    ...
                    picturelist.append(
                        ["https://instagram.com" + link, profilepic, 1.0])
                except:
