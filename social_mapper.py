...
def fill_facebook(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("Facebook Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rFacebook Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        ...
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = FacebookfinderObject.getFacebookProfiles(
                    person.first_name, person.last_name, facebook_username, facebook_password)
            except:
                continue
        else:
            continue
            ...
            if image_link:
                try:
                    ...
                    response = requests.get(
                        image_link, cookies=cookies, headers=headers, stream=True)
                    ...
                    del response
                    potential_target_image = face_recognition.load_image_file(
                        "potential_target_image")
                    try:  # try block for when an image has no faces
                        potential_target_encoding = face_recognition.face_encodings(
                            potential_target_image)[0]
                    except:
                        continue
                    results = face_recognition.face_distance(
                        [target_encoding], potential_target_encoding)
                    for result in results:
                        ...
                        if args.mode == "fast":
                            if result < threshold:
                                person.facebook = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.facebookimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                person.facebookcdnimage = encoding.smart_str(
                                    cdnpicture, encoding='ascii', errors='ignore')
                                if args.vv == True:
                            ...
                            if result < threshold:
                                ...
                                updatedlist.append(
                                    [profilelink, image_link, result, cdnpicture])
        ...
        if args.mode == "accurate":
            ...
            if highestdistance < threshold:
                person.facebook = encoding.smart_str(
                    bestprofilelink, encoding='ascii', errors='ignore')
                person.facebookimage = encoding.smart_str(
                    bestimagelink, encoding='ascii', errors='ignore')
                person.facebookcdnimage = encoding.smart_str(
                    bestcdnpicture, encoding='ascii', errors='ignore')
                if args.vv == True:
                    print("\tMatch found: " + person.full_name)
                    print("\tFacebook: " + person.facebook)
    try:
...
def fill_twitter(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("Twitter Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rTwitter Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = TwitterfinderObject.getTwitterProfiles(
            ...
            if image_link:
                try:
                    urllib.request.urlretrieve(
                        image_link, "potential_target_image")
                    potential_target_image = face_recognition.load_image_file(
                        "potential_target_image")
                    try:  # try block for when an image has no faces
                        potential_target_encoding = face_recognition.face_encodings(
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.twitter = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.twitterimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                if args.vv == True:
                                    print("\tMatch found: " + person.full_name)
...
def fill_instagram(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("Instagram Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rInstagram Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        ...
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = InstagramfinderObject.getInstagramProfiles(
                    person.first_name, person.last_name, instagram_username, instagram_password)
            ...
            if image_link:
                try:
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.instagram = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
...
def fill_linkedin(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("LinkedIn Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rLinkedIn Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        ...
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = LinkedinfinderObject.getLinkedinProfiles(
                    person.first_name, person.last_name, linkedin_username, linkedin_password)
            except:
                continue
        else:
            continue
            ...
            if image_link:
                try:
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.linkedin = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.linkedinimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                if args.vv == True:
                                    print("\tMatch found: " + person.full_name)
...
def fill_vkontakte(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("VKontakte Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rVKontakte Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = VkontaktefinderObject.getVkontakteProfiles(
                    person.first_name, person.last_name)
            except:
                continue
        else:
            ...
            if image_link:
                try:
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.vk = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.vkimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                if args.vv == True:
                                    print("\tMatch found: " + person.full_name)
...
def fill_weibo(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("Weibo Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rWeibo Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = WeibofinderObject.getWeiboProfiles(
                    person.first_name, person.last_name)
            except:
                continue
        else:
            ...
            if image_link:
                try:
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.weibo = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.weiboimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                if args.vv == True:
                                    print("\tMatch found: " + person.full_name)
...
def fill_douban(peoplelist):
    ...
    for person in peoplelist:
        if args.vv == True:
            print("Douban Check %i/%i : %s" %
                  (count, ammount, person.full_name))
        else:
            sys.stdout.write("\rDouban Check %i/%i : %s                                " %
                             (count, ammount, person.full_name))
            sys.stdout.flush()
        count = count + 1
        if person.person_image:
            try:
                ...
                target_encoding = face_recognition.face_encodings(target_image)[
                    0]
                profilelist = DoubanfinderObject.getDoubanProfiles(
                    person.first_name, person.last_name)
            except:
                continue
        else:
            ...
            if image_link:
                try:
                    ...
                    for result in results:
                        if args.mode == "fast":
                            if result < threshold:
                                person.douban = encoding.smart_str(
                                    profilelink, encoding='ascii', errors='ignore')
                                person.doubanimage = encoding.smart_str(
                                    image_link, encoding='ascii', errors='ignore')
                                if args.vv == True:
                                    print("\tMatch found: " + person.full_name)
...
def login():
    ...
    opener = urllib.request.build_opener(urllib.request.HTTPRedirectHandler(), urllib.request.HTTPHandler(
        debuglevel=0), urllib.request.HTTPSHandler(debuglevel=0), urllib.request.HTTPCookieProcessor(cookiejar))
    ...
    page = loadPage(
        opener, "https://www.linkedin.com/checkpoint/lg/login-submit", login_data)
...
parser.add_argument('-s', '--showbrowser', action='store_true',
                    dest='showbrowser', help='If flag is set then browser will be visible')

parser.add_argument('-a', '--all', action='store_true',
                    dest='a', help='Flag to check all social media sites')
parser.add_argument('-fb', '--facebook', action='store_true',
                    dest='fb', help='Flag to check Facebook')
parser.add_argument('-tw', '--twitter', action='store_true',
                    dest='tw', help='Flag to check Twitter')
parser.add_argument('-ig', '--instagram', action='store_true',
                    dest='ig', help='Flag to check Instagram')
parser.add_argument('-li', '--linkedin', action='store_true', dest='li',
                    help='Flag to check LinkedIn - Automatic with \'company\' input type')
parser.add_argument('-vk', '--vkontakte', action='store_true',
                    dest='vk', help='Flag to check the Russian VK VKontakte Site')
parser.add_argument('-wb', '--weibo', action='store_true',
                    dest='wb', help='Flag to check the Chinese Weibo Site')
parser.add_argument('-db', '--douban', action='store_true',
                    dest='db', help='Flag to check the Chinese Douban Site')
...
if args.format == "csv":
    ...
    for full_name, person_image in filereader:
        try:
            full_name = encoding.smart_str(
                full_name, encoding='ascii', errors='ignore')
            person_image = encoding.smart_str(
                person_image, encoding='ascii', errors='ignore')
            # print person_image
            urllib.request.urlretrieve(
                person_image, "temp-targets/" + full_name + ".jpg")
            first_name = full_name.split(" ")[0]
            last_name = full_name.split(" ", 1)[1]
            person = Person(first_name, last_name, full_name,
                            "temp-targets/" + full_name + ".jpg")
            person.person_imagelink = person_image
            peoplelist.append(person)
        except Exception as e:
            print(
                "Error getting image or creating person structure, skipping:" + full_name)
    ...
    for filename in os.listdir(args.input):
        if filename.endswith(".jpg") or filename.endswith(".png")or filename.endswith(".jpeg"):
            ...
            try:
                ...
                last_name = ""
            first_name = encoding.smart_str(
                first_name, encoding='ascii', errors='ignore')
            last_name = encoding.smart_str(
                last_name, encoding='ascii', errors='ignore')
            full_name = encoding.smart_str(
                full_name, encoding='ascii', errors='ignore')
            person = Person(first_name, last_name,
                            full_name, args.input + filename)
...
if args.format == "company":
    ...
    if args.companyid is not None:  # Don't find company id, use provided id from -cid or --companyid flag
        ...
        headers = {'Csrf-Token': 'ajax:0397788525211216808',
    ...
    try:
        ...
        print(
            "Company has no people listed, nothing to match against other Social Networks!")
        sys.exit(0)
        ...
        for c in content['elements'][0]['elements']:
            if 'com.linkedin.voyager.search.SearchProfile' in c['hitInfo'] and c['hitInfo']['com.linkedin.voyager.search.SearchProfile']['headless'] == False:
                try:
                    ...
                    rooturl = c['hitInfo']['com.linkedin.voyager.search.SearchProfile'][
                    ...
                    person_image = encoding.smart_str(
                        person_image, encoding='ascii', errors='ignore')
                    linkedin = "https://www.linkedin.com/in/%s" % c['hitInfo'][
                        'com.linkedin.voyager.search.SearchProfile']['miniProfile']['publicIdentifier']
                    linkedin = encoding.smart_str(
                        linkedin, encoding='ascii', errors='ignore')
                    urllib.request.urlretrieve(
                        person_image, "temp-targets/" + full_name + ".jpg")
                    person = Person(first_name, last_name, full_name,
                                    "temp-targets/" + full_name + ".jpg")
...
if args.format == "socialmapper":
    ...
    for personhtml in tbodylist:
        person_image = encoding.smart_str(personhtml.findAll(
            "td")[0].string, encoding='ascii', errors='ignore').replace(";", "")
        full_name = encoding.smart_str(personhtml.findAll(
            "td")[1].string, encoding='ascii', errors='ignore')
        first_name = full_name.split(" ")[0]
        last_name = full_name.split(" ", 1)[1]
        urllib.request.urlretrieve(
            person_image, "temp-targets/" + full_name + ".jpg")
        person = Person(first_name, last_name, full_name,
                        "temp-targets/" + full_name + ".jpg")
        person.person_imagelink = person_image
        person.linkedin = encoding.smart_str(personhtml.findAll("td")[2].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.linkedinimage = encoding.smart_str(personhtml.findAll("td")[2].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.facebook = encoding.smart_str(personhtml.findAll("td")[3].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.facebookimage = encoding.smart_str(personhtml.findAll("td")[3].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.facebookcdnimage = encoding.smart_str(personhtml.findAll("td")[3].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.twitter = encoding.smart_str(personhtml.findAll("td")[4].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.twitterimage = encoding.smart_str(personhtml.findAll("td")[4].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.instagram = encoding.smart_str(personhtml.findAll("td")[5].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.instagramimage = encoding.smart_str(personhtml.findAll("td")[5].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.vk = encoding.smart_str(personhtml.findAll("td")[7].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.vkimage = encoding.smart_str(personhtml.findAll("td")[7].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.weibo = encoding.smart_str(personhtml.findAll("td")[8].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.weiboimage = encoding.smart_str(personhtml.findAll("td")[8].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        person.douban = encoding.smart_str(personhtml.findAll("td")[9].find(
            "a")['href'], encoding='ascii', errors='ignore').replace(";", "")
        person.doubanimage = encoding.smart_str(personhtml.findAll("td")[9].find(
            "img")['src'], encoding='ascii', errors='ignore').replace(";", "")
        peoplelist.append(person)
...
if not args.format == "linkedint" and not args.format == "company":
    if args.a == True or args.li == True:
        if not (linkedin_username == "" or linkedin_password == ""):
            ...
            print(
                "Please provide LinkedIn Login Credentials in the social_mapper.py file")
if args.a == True or args.vk == True:
    if not (vk_username == "" or vk_password == ""):
        peoplelist = fill_vkontakte(peoplelist)
    else:
        print(
            "Please provide VK (VKontakte) Login Credentials in the social_mapper.py file")
...
outputfilename = "SM-Results/" + \
    args.input.replace("\"", "").replace("/", "-") + "-social-mapper.csv"
phishingoutputfilename = "SM-Results/" + \
...
if args.a == True or args.li == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewriterlinkedin = open(phishingoutputfilenamelinkedin.format(
            phishingoutputfilenamelinkedin), 'w')
if args.a == True or args.fb == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewriterfacebook = open(phishingoutputfilenamefacebook.format(
            phishingoutputfilenamefacebook), 'w')
if args.a == True or args.tw == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewritertwitter = open(phishingoutputfilenametwitter.format(
            phishingoutputfilenametwitter), 'w')
if args.a == True or args.ig == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewriterinstagram = open(phishingoutputfilenameinstagram.format(
            phishingoutputfilenameinstagram), 'w')
if args.a == True or args.vk == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewritervkontakte = open(phishingoutputfilenamevkontakte.format(
            phishingoutputfilenamevkontakte), 'w')
if args.a == True or args.wb == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewriterweibo = open(phishingoutputfilenameweibo.format(
            phishingoutputfilenameweibo), 'w')
if args.a == True or args.db == True or args.format == "socialmapper":
    ...
    if args.email is not None:
        ...
        filewriterdouban = open(phishingoutputfilenamedouban.format(
            phishingoutputfilenamedouban), 'w')
...
htmloutputfilename = "SM-Results/" + \
    args.input.replace("\"", "").replace("/", "-") + "-social-mapper.html"
if args.format == "imagefolder":
...
for person in peoplelist:
    body = "<tbody>" \
        ...
        "</tr>" \
        "</tbody>" % (person.person_imagelink, person.person_imagelink, person.full_name, person.linkedin, person.linkedinimage, person.linkedin, person.facebook, person.facebookcdnimage, person.facebook, person.twitter, person.twitterimage,
                      person.twitter, person.instagram, person.instagramimage, person.instagram, "", "", "", person.vk, person.vkimage, person.vk, person.weibo, person.weiboimage, person.weibo, person.douban, person.doubanimage, person.douban)
...
outputfoldername = "SM-Results/" + \
    args.input.replace("\"", "").replace("/", "-") + "-social-mapper"
if args.format != "imagefolder":
